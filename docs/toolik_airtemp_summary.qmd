...
```{r}
toolik_weather <- toolik_weather %>% clean_names() # What does this do? Say it in words. Will store it as the same name. 
...
# Now what are the column names? Now they are all in lowercase 
```
...
# i notice that the names are not great, uppercases and spaces 
```
...
```{r}
# cleaning up a the column active chlorophyll ----

# when looking at data, we see that instead of "NA"s, it uses "." We need to fix that 

toolik_chlorophyll$active_chl_a_ug_l <- na_if(toolik_chlorophyll$active_chl_a_ug_l, ".")


#when looks at this column in the environment, we see that it is stored as a character not numeric 
...
ggplot(data = toolik_chlorophyll,
  ...
  scale_color_gradient(low = "blue", high = "orange") +
  theme_minimal() +
  labs(title = "Toolik Chlorophyll Concentrations",
       x = "Lake depth (m)",
       y = "Active chlorophyll concentration (mg/L)")
```
